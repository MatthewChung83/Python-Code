...
add_src = add_src.loc[add_src["result"] != 0].reset_index(drop=True)
ct_src = pd.read_csv(r'C:\Py_Project\project\LandMoi\city_index.csv').drop_duplicates()
CsvToPath = r'C:\Py_Project\output\Add2LandNo.csv'

target_file = r'C:\Py_Project\output\Add2LandNo.csv'

if os.path.isfile(target_file):    
    add_out = pd.read_csv(r'C:\Py_Project\output\Add2LandNo.csv',error_bad_lines=False).query("i != 'i'").drop_duplicates()
    start = len(add_out)
else:
    start = 0
    LandDataTitle = ('i','eaid','ID','addressi','casei','LandNo_f','LandNo_e','District','GeoOffice','Lot','LandNo','AreaSq','LdValue','LdPrice','city','town','road','lane','alley','no')
    ReadCsv(CsvToPath,LandDataTitle)

for i in range(start,len(add_src)):
#for i in range(151,152):
    eaid = DeColumn(i,'eaid',add_src)
    ID = DeColumn(i,'ID',add_src)
    addressi = DeColumn(i,'addressi',add_src)
    casei = DeColumn(i,'casei',add_src)
    city_in = DeColumn(i,'city',add_src)
    town_in = DeColumn(i,'town',add_src)
    road_in = DeColumn(i,'Street',add_src)
    lane_in = DeColumn(i,'lane',add_src)
    alley_in = DeColumn(i,'alley',add_src)
    no_in = DeColumn(i,'No',add_src)+DeColumn(i,'floor',add_src)
    ...
    if '錯誤' in CoordXY.text:
        LandData = (i,eaid,ID,addressi,casei,'網頁錯誤')
        print(LandData)
        continue
    ...
    if '錯誤' in DoorInfoByXY.text:
        LandData = (i,eaid,ID,addressi,casei,'系統已達上限')
        print(LandData)
        break
    
    elif DoorInfoByXY.text == 'null':
        LandData = (i,eaid,ID,addressi,casei,'查無地號')
        ...
        if '查無此宗地屬性' in LandDesc.text:
            ...
            LandData = (i,eaid,ID,addressi,casei,LandNo_f,LandNo_e,District,GeoOffice,Lot,LandNo,AreaSq,LdValue,LdPrice,city_in,town_in,road_in,lane_in,alley_in,no_in)
            ReadCsv(CsvToPath,LandData)
            print(LandData)
    time.sleep(0.1)