...
db_acct = 'pyuser'
db_pwd = 'Ucredit7607'
wk_tb = 'ElAddrTblandNo78'
df_tb = 'ElAddrTblandNo'
df_tb = 'ElAddrTbLandNoF'
id_start = 0
id_end = 400000
...
def src_in(table,pname,offobs,nextobs):
    ...
    and [{pname}] > (select top 1 [{pname}]  from [UIS].[dbo].[{df_tb}] order by [{pname}]  desc)    
    and [{pname}] not in (select [{pname}] from [UIS].[dbo].[{df_tb}])
    ORDER BY [{pname}]
...
c_src = src_in('ElAddrTbCheckF','eaid',0,200000)
...
for i in range(len(c_src)):
    ...
    if isrecexist(df_tb,'eaid',eaid) > 0 and i != len(c_src) - 1:
        continue
    elif isrecexist(df_tb,'eaid',eaid) > 0 and i == len(c_src) - 1:
        break
    else:
        pass
    ...
    if '網頁發生錯誤' in CoordXY.text:
        ...
        LdPrice = ''
        result = 'N'
        inserttodb(df_tb,eaid,ID,addressi,casei,LandNo_f,LandNo_b,District,GeoOffice,Lot,LandNo,AreaSq,LdValue,LdPrice,datatime,result)
        continue
