...
try:
    restart()
    time.sleep(20)
    pingtest = os.system('ping -c 1 google.com')
    while pingtest !=0:            
        print('internet is down! waiting 5s...')
        time.sleep(5)
        pingtest = os.system('ping -c 1 google.com')
    print('internet is up!')
except:
    print('except:','adsl pppoe restart err! waiting 20s...')

    
c_src = src_in('ElAddrTbCheck','eaid',0,100000)
...
for i in range(len(c_src)):
    ...
    try:
        ...
        print('except: ',i,eaid,'CoordXY',time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
        restart()
        time.sleep(40)
    if '網頁發生錯誤' in CoordXY.text:
        ...
        try:
            ...
            print('except: ',i,eaid,'DoorInfoByXY',time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
            restart()
            time.sleep(40)    
        if '網頁發生錯誤' in DoorInfoByXY.text:
            ...
            try:
                ...
                print('except: ',i,eaid,'LandDesc',time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
                restart()
                time.sleep(40)
