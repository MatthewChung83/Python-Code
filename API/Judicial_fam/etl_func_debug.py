...
def src_obs(server,username,password,database,fromtb,totb,check):
    ...
    where id in (select ID from judicial_fam where note2 not like '%年%' and note2 like '%死亡%' and note2 not like '%""%')  
...
def dbfrom(server,username,password,database,fromtb,totb,check):
    ...
    script = f"""
    select distinct 0 as personi,ID,name,0 as casei,0 as type,0 as c,0 as m,0 as age,'' as flg,rowid
    into #test
    from [{totb}]
    where id in (select ID from judicial_fam where note2 not like '%年%' and note2 like '%死亡%' and note2 not like '%""%')
    and update_date < '2023/01/12'
