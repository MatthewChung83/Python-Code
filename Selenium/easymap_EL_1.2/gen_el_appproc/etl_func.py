...
def dbfromel03dtl(server,username,password,database,fromtb,Machine):
    import pymssql
    conn = pymssql.connect(server=server, user=username, password=password, database = database)
    cursor = conn.cursor()
    ...
    script = f"""
    select [EntityType], [Machine], [EntityPhase], [EntityPath], [EntityObs], [validtrans], [Status], [actiondttm], [Statusdttm]
    from [{fromtb}] where Machine = '{Machine}' and status <> 'Done' and Entitytb = 'EL03DTL' and status <> 'Done'
    order by EntityType
    """
    cursor.execute(script)
    batch_index = cursor.fetchall()
